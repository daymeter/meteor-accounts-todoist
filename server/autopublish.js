...
Accounts.addAutopublishFields({
    ...
    forLoggedInUser: _.map(
        ...
        Todoist.whitelistedFields.concat(['accessToken', 'expiresAt']), // don't publish refresh token
        function (subfield) { return 'services.todoist.' + subfield; }),

    forOtherUsers: _.map(
        // even with autopublish, no legitimate web app should be
        // publishing all users' emails
        _.without(Todoist.whitelistedFields, 'emails'),
        function (subfield) { return 'services.todoist.' + subfield; })
});
